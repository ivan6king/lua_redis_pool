...
conf.redis = {
    ...
    max_idle_timeout = 1000,        -- 连接最大空闲时间(ms) 如果pool_size=nil 此配置无效
    pool_size = nil,                -- 单nginx worker进程最大的连接池大小 如果不需要使用连接池,配置为nil即可
}

-- demo2:使用常规操作(不使用连接池 不兼容lua-resty-redis的订阅、发布命令)
-- redis的专长不是处理订阅、发布场景。如有订阅、发布发布场景请使用专业的mq(rabbitmq/kafka等等)
-- 如果必须使用订阅命令,请参考demo1
local red = redis:new(conf.redis)

--常规操作
red:set("name","lua_redis_pool(redis demo2)")
